...
impl RtcState {
    pub async fn initialize(router: &Router, init_data: InitializationInput) -> Result<Self, ()> {
        ...
        webrtc_options.enable_sctp = true;
    ...
    pub fn get_init_data(&self) -> TransportInitData {
        match &self.transport_mode {
            ...
            TransportMode::CombinedRtp(transport) => TransportInitData::CombinedRtp {
                id: transport.id(),
                port: transport.tuple().local_port(),
                protocol: transport.tuple().protocol(),
                ip: RTC_IPS[0].announced_ip.unwrap_or(RTC_IPS[0].ip),
    ...
    pub fn get_webrtc_transport_by_id(&self, id: TransportId) -> Option<&WebRtcTransport> {
        match self.transport_mode {
            ...
            TransportMode::CombinedWebRtc(_) => todo!(),
            TransportMode::CombinedRtp(_) => todo!(),
        }
    }

    pub fn get_rpt_transport_by_id(&self, id: TransportId) -> Option<&PlainTransport> {
        match self.transport_mode {
        ...
        match self.transport_mode {
            TransportMode::SplitWebRtc(..) | TransportMode::CombinedWebRtc(..) => {
                if let ConnectTransportParams::WebRtc { dtls_parameters } = &connect_data.params {
                    let transport = self.get_webrtc_transport_by_id(connect_data.id).ok_or(())?;
            ...
            TransportMode::CombinedRtp(_) => {
                if let ConnectTransportParams::Rtp { srtp_parameters } = &connect_data.params {
                    let transport = self.get_rpt_transport_by_id(connect_data.id).ok_or(())?;
    ...
    pub(crate) fn stop_consume(&mut self, consumer_id: &ConsumerId) -> Result<(), ()> {
        let _ = self.consumers.remove(consumer_id).ok_or(|| ());
        Ok(())
    }
    ...
    pub async fn set_consumer_pause(
        ...
        let consumer = self.consumers.get_mut(consumer_id).ok_or(())?;
