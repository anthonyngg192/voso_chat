...
impl<'r> RoomUsers {
    ...
    pub async fn generate_room(&'r self, id: String) -> Result<UserGuard<'r>, ApiError> {
        let token = {
            let registration = self.room.registrations.read().await;
            let mut rng = thread_rng();
            let mut token = generate_token(&mut rng)?;
            while registration.contains_key(&token) {
